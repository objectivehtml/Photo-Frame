...
//$settings['photo_frame_show_editor_lv'] && $this->low_variables ||
		...
		$directory = $this->EE->photo_frame_model->get_directory($this->settings['photo_frame_upload_group']);
		...
		foreach($photos as $index => $row)
		{
			...
			{	
				...
				{							
					...
					$return[$index]['total_photos']   = !$params['limit'] ? $return[$index]['grand_total_photos'] : ($params['limit'] > $return[$index]['grand_total_photos'] ? $return[$index]['grand_total_photos'] : $params['limit']);
		...
		if(is_array($post))
		{
    		foreach($post as $index => $photo)
    		{
    		    if(isset($photo['new']))
    		    {
    				...
    				$photo['exif_data'] 	 	 = !is_string($photo['exif_data']) ? json_encode($photo['exif_data']) : $photo['exif_data'];
	...
	public function delete($ids)
	{
		...
		$this->EE->photo_frame_model->delete_entries($ids, FALSE, $this->settings);
	}
	...
	private function _get_post()
	{
		...
		{
			$post = $this->EE->input->post('field_id_'.$this->settings['grid_field_id'], TRUE);
